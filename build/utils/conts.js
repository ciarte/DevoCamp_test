...
function createEmail(name, req) {
    const endpoint = req.originalUrl;
    if (endpoint === '/postulaciones')
        return `Estimado ${name}, Mensaje para la ruta 1`;
    else if (endpoint === '/empresas/')
        return `Estimado ${name}, Mensaje para la ruta 2`;
    else
        throw new Error(`Endpoint '${endpoint}' no es válido`);
}
